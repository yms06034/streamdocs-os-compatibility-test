...
ENV RL_VER=9.5 \
    RL_GPG=https://dl.rockylinux.org/pub/rocky/RPM-GPG-KEY-Rocky-9 \
    RL_BASEOS=http://dl.rockylinux.org/vault/rocky/9.5/BaseOS/x86_64/os/ \
    RL_APPSTR=http://dl.rockylinux.org/vault/rocky/9.5/AppStream/x86_64/os/ \
    RL_CRB=http://dl.rockylinux.org/vault/rocky/9.5/CRB/x86_64/os/ \
    RL_EXTRAS=http://dl.rockylinux.org/vault/rocky/9.5/extras/x86_64/os/
...
RUN set -eux; \
  dnf -y --releasever=${RL_VER} --disablerepo='*' --setopt=reposdir=/dev/null \
    ...
    --setopt=crb.gpgcheck=1 --setopt=crb.gpgkey=${RL_GPG} \
