...
RUN set -eux; \
  ...
  echo 'name=Rocky Linux 9.0 - BaseOS' >> /etc/yum.repos.d/rocky.repo && \
  echo 'baseurl=http://dl.rockylinux.org/vault/rocky/9.0/BaseOS/$basearch/os/' >> /etc/yum.repos.d/rocky.repo && \
  ...
  echo 'name=Rocky Linux 9.0 - AppStream' >> /etc/yum.repos.d/rocky.repo && \
  echo 'baseurl=http://dl.rockylinux.org/vault/rocky/9.0/AppStream/$basearch/os/' >> /etc/yum.repos.d/rocky.repo && \
  echo 'gpgcheck=1' >> /etc/yum.repos.d/rocky.repo && \
  echo 'enabled=1' >> /etc/yum.repos.d/rocky.repo && \
  echo 'gpgkey=https://dl.rockylinux.org/pub/rocky/RPM-GPG-KEY-Rocky-9' >> /etc/yum.repos.d/rocky.repo

RUN microdnf install -y --releasever=9.0 \
    coreutils-single \
    procps-ng \
    wget \
    curl \
