...
RUN set -eux; \
  ...
  echo 'name=Rocky Linux 8.5 - BaseOS' >> /etc/yum.repos.d/rocky.repo && \
  echo 'baseurl=http://dl.rockylinux.org/vault/rocky/8.5/BaseOS/$basearch/os/' >> /etc/yum.repos.d/rocky.repo && \
  ...
  echo 'name=Rocky Linux 8.5 - AppStream' >> /etc/yum.repos.d/rocky.repo && \
  echo 'baseurl=http://dl.rockylinux.org/vault/rocky/8.5/AppStream/$basearch/os/' >> /etc/yum.repos.d/rocky.repo && \
  echo 'gpgcheck=1' >> /etc/yum.repos.d/rocky.repo && \
  echo 'enabled=1' >> /etc/yum.repos.d/rocky.repo && \
  echo 'gpgkey=https://dl.rockylinux.org/pub/rocky/RPM-GPG-KEY-rockyofficial' >> /etc/yum.repos.d/rocky.repo

RUN dnf install -y --releasever=8.5 \
    coreutils-single \
    procps-ng \
    wget \
    curl \
